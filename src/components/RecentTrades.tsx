        ...
        <div className="w-full bg-bg-panel border border-border rounded-3xl p-3 flex flex-col h-full overflow-hidden">
            ...
            <div className="flex flex-col gap-0.5 overflow-y-auto no-scrollbar flex-1 min-h-0">
                ...
                {!loading && trades.map((trade, i) => (
                    <div key={trade.id || i} className="flex items-center justify-between py-1 px-1 hover:bg-white/5 cursor-pointer rounded text-[11px] font-geist leading-4">
                        ...
                        <span className="flex-1 text-right text-[#BBBBBB]">{parseFloat(trade.size).toFixed(4)}</span>
