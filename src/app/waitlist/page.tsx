...
import { getWaitlistStatus, joinWaitlist } from '@/services/api'
import {
  getAuthToken,
  useDynamicContext,
  useIsLoggedIn,
...
export default function WaitlistPage() {
  ...
  // Check if user is already on waitlist when they connect
  useEffect(() => {
    const checkStatus = async () => {
      if (isLoggedIn) {
        ...
        try {
          ...
          const status = await getWaitlistStatus(authToken ?? undefined)
          setIsWaitlisted(status.isWaitlisted)
        } catch {
          // Ignore errors
        } finally {
  ...
  const handleJoinWaitlist = async () => {
    ...
    try {
      const authToken = getAuthToken()
      const result = await joinWaitlist(authToken ?? undefined)
      if (result.success) {
  ...
  const description = isWaitlisted
    ? "We'll notify you when access opens. Thanks for your interest in Tide."
  ...
  const primaryButtonContent = isWaitlisted ? (
    <>
      <CheckIcon />
      ...
      <AuthCard>
        <div className="relative z-10 flex flex-col items-start h-full justify-between">
          ...
          <div className="w-full">
            <AuthButton onClick={handleJoinWaitlist} className="w-full">
